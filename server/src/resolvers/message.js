...
// lowDB를 사용하면 readDB 대신 db를 바로 불러옴
import db from '../dbController.js';

// ** GraphQL에서 lowDB를 쓰는 경우 'db'를 전부 'models'로 교체
// (server/index.js의 ApolloServer의 context에서 설정한 대로)
// LowDB 사용하면 writeDB 없이 그냥 models.write() 쓰면 됨
...
const messageResolver = {
  Query: {
    ...
    messages: (parent, { cursor = '' }, { models }) => {
      ...
      // models.messages가 없을 경우 빈 배열([]) 반환
  ...
  Mutation: {
    createMessage: (parent, { text, userId }, { models }) => {
      ...
      // (msgs 대신 models.messages)
      models.messages.unshift(newMsg); // messages DB의 맨 앞에 새 메시지를 추가

      // lowDB 사용하면 setMsgs 대신 바로 db에 기록
      // setMsgs(models.messages); // DB 수정 (setMsgs의 역할은 writeDB)
      db.write();
    ...
    updateMessage: (parent, { id, text, userId }, { models }) => {
      ...
      const targetIndex = models.messages.findIndex((msg) => msg.id === id); // '인덱스'를 찾아야 하므로 findIndex
      ...
      // lowDB 사용하면 setMsgs 대신 바로 db에 기록
      // setMsgs(models.messages); // 새로 만들어진 배열로 DB 수정 (writeDB)
      db.write();
    ...
    deleteMessage: (parent, { id, userId }, { models }) => {
      ...
      const targetIndex = models.messages.findIndex((msg) => msg.id === id); // 삭제할 메시지를 index로 가져옴
      ...
      // lowDB 사용하면 setMsgs 대신 바로 db에 기록
      // setMsgs(models.messages); // 수정된 배열로 DB 수정
      db.write();
  ...
  Message: {
    // user라는 필드에 models.users[msg.userId] 값 반환
